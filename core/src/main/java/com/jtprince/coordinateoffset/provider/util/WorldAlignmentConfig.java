...
public class WorldAlignmentConfig {
    ...
    public static WorldAlignmentConfig DEFAULT = new WorldAlignmentConfig(new ArrayList<>(
            List.of(new Alignment("world", "world_nether", 3)
    )));

    public void serializeTo(SequencedMap<String, Object> providerConfig) {
        List<String> configStringList = new ArrayList<>();
        for (Alignment alignment : alignments) {
            StringBuilder sb = new StringBuilder();
            sb.append(alignment.greaterWorldName).append(":").append(alignment.lesserWorldName);
            if (alignment.scaleShift != 0) {
                sb.append(":").append(1 << alignment.scaleShift);
            }
            configStringList.add(sb.toString());
        }
        providerConfig.put("worldAlignment", configStringList);
    }

    public static WorldAlignmentConfig deserialize(List<String> configStringList) {
