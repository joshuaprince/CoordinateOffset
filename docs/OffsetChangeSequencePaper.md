...
repeated runs. For example, standard logging shows PlayerJoinEvent always fires before the packet JOIN_GAME. But
randomized delays expose concurrency: by joining the server multiple times, the log order of these events and packets
change. We can drill even further into the sequence by adding thread-only breakpoints in PlayerJoinEvent. Thread-only
breakpoints introduce even more delay and show that more packets following JOIN_GAME are _also_ concurrent with 
PlayerJoinEvent.
...
On world change:
  ...
  ------------------------------              -------------------------
   ...
   PlayerChangedWorldEvent ◄── concurrent ─┬►  RESPAWN
                                           ├►  UNLOAD_CHUNK (1+) (NB: Must use old offset!)
                                           ├►  PLAYER_POSITION_AND_LOOK
                                           ├►  UPDATE_VIEW_POSITION
                                           ├►  INITIALIZE_WORLD_BORDER
                                           └►  SPAWN_POSITION
...
On world change: (Same as 1.21.9)
On respawn after death: (Same as 1.21.9)
```
...
In 1.21.9, Paper deprecated PlayerSpawnLocationEvent and introduced AsyncPlayerSpawnLocationEvent. The async event is
fired at the right time to generate an offset. However, the async event is not a suitable hook because it contains no
Player object or any player data beyond a UUID.

PlayerJoinEvent is therefore the only suitable event to use to generate the joining player's first offset. However,
it requires a hack to work around the concurrency. The strategy 1.21.9+ uses is:
 - Use PlayerJoinEvent to generate an offset.
 - **Block** the Netty thread from sending a JOIN_GAME packet until an offset is generated.
...
Then, both respawns and world changes send a PLAYER_POSITION_AND_LOOK packet, which triggers the offset swap.
